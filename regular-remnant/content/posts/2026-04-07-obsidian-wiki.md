...
UPDATE: I recently added two pieces to this wiki project. The first is a per-project task layer: each project directory maps to tasks.md and tasks-done.md files inside the vault, and a small set of skills (task-inbox, task-plan, task-next, task-done, task-review, task-defer) lets Claude Code capture, decompose, surface, and close tasks from any directory without reaching for an external tracker. The second is [QMD](https://github.com/tobi/qmd) integration: a local semantic search index over the vault's markdown that wiki skills like wiki-query use in place of grep when it's available, so Claude can retrieve pages by meaning instead of exact keyword matches, with a plain-text fallback when the index isn't built. 
